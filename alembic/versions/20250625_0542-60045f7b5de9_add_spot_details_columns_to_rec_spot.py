...
def upgrade() -> None:
    ...
    op.drop_index("idx_rec_spot_coordinates", table_name="rec_spot", if_exists=True)
    op.drop_index("idx_rec_spot_time_slot", table_name="rec_spot", if_exists=True)
...
def downgrade() -> None:
    ...
    op.add_column(
        "rec_spot",
        sa.Column(
            "google_map_image_url", sa.TEXT(), autoincrement=False, nullable=True
        ),
    )
    op.add_column(
        "rec_spot",
    ...
    op.add_column(
        "rec_spot",
        sa.Column(
            "time_slot", sa.VARCHAR(length=10), autoincrement=False, nullable=True
        ),
    )
    op.add_column(
        "rec_spot",
    ...
    op.create_index(
        "idx_rec_spot_coordinates", "rec_spot", ["latitude", "longitude"], unique=False
    )
    # ### end Alembic commands ###
