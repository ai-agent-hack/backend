...
Revision ID: 46d3cf9edb8e
Revises: 
Create Date: 2025-06-26 06:53:35.047421+00:00
...
def upgrade() -> None:
    ...
    op.create_index('idx_rec_spot_plan_version', 'rec_spot', ['plan_id', 'version'], unique=False)
    op.create_index('idx_rec_spot_rank', 'rec_spot', ['plan_id', 'version', 'rank'], unique=False)
    ...
    sa.Column('google_maps_data', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('plan_id', 'version', name='uq_routes_plan_version')
    )
    op.create_index('idx_routes_plan_version', 'routes', ['plan_id', 'version'], unique=False)
    ...
    sa.Column('participants_count', sa.Integer(), server_default='2', nullable=False),
    ...
    op.create_index('idx_pre_info_dates', 'pre_infos', ['start_date', 'end_date'], unique=False)
    ...
    op.create_index(op.f('ix_pre_infos_user_id'), 'pre_infos', ['user_id'], unique=False)
    ...
    sa.Column('ordered_spots', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
    ...
    op.create_index('idx_route_days_route_id', 'route_days', ['route_id', 'day_number'], unique=False)
    op.create_index(op.f('ix_route_days_id'), 'route_days', ['id'], unique=False)
    op.create_table('rec_plan',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('plan_id', sa.String(length=50), nullable=False),
    sa.Column('version', sa.Integer(), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['pre_info_id'], ['pre_infos.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_rec_plan_plan_id', 'rec_plan', ['plan_id'], unique=False)
    op.create_index('idx_rec_plan_version', 'rec_plan', ['plan_id', 'version'], unique=True)
    ...
    sa.Column('directions_steps', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.ForeignKeyConstraint(['route_day_id'], ['route_days.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_route_segments_day_order', 'route_segments', ['route_day_id', 'segment_order'], unique=False)
